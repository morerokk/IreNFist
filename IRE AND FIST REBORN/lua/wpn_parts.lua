...
if BeardLib.Utils:ModLoaded("BipodG36") then --Permanently expanded, but functions properly
...
--]]
	...
	self.parts.inf_fnfal_dmrkit.stats.extra_ammo = 30
	...
	self.parts.inf_fnfal_classickit.stats.extra_ammo = 30
	...
	self.parts.wpn_fps_ass_g3_b_sniper.stats.extra_ammo = 17
...
if BeardLib.Utils:ModLoaded("BipodG3") then
	...
	-- M308 PARTS
	-- Abraham Body
	self.parts.wpn_fps_ass_m14_body_ebr.stats = {spread = -15, recoil = 12, concealment = 2}
	-- Jaeger Body
	...
	self.parts.wpn_fps_ass_m14_body_ruger.stats = { spread = -20, recoil = -5, concealment = 5}
	-- Scope Mount
	...
	self:convert_part("inf_m308_20rnd", "dmr", "hrifle", nil, nil, 600, 700)
...
if BeardLib.Utils:ModLoaded("BipodM14") then
	...
	--
if BeardLib.Utils:ModLoaded("Bipod WA2000") then --Doesn't work properly, gives standard lion instead of keeping the built-in bipod, the expanded bipod doesn't work properly either
...
if BeardLib.Utils:ModLoaded("MP5K FG") then
	...
	self.parts.inf_c96_auto.custom_stats = {inf_rof_mult = 1000/600, anim_speed_mult = 600/1000, sdesc2 = "action_shortrecoilmod"}
...
if BeardLib.Utils:ModLoaded("Vanilla Styled Weapon Mods") and self.parts.wpn_fps_pis_lebman_body_classic then
	...
	self:convert_part("wpn_fps_aug_b_big", "lrifle", "ldmr", nil, nil, nil, nil)
...
if BeardLib.Utils:ModLoaded("Vanilla Styled Weapon Mods Volume 2") and self.parts.wpn_fps_shot_minibeck_shells then
	...
	self.parts.wpn_fps_smg_sterling_b_poke.custom_stats = deep_clone(barrel_m1)
	self.parts.wpn_fps_pis_breech_b_length.custom_stats = deep_clone(barrel_m1)
...
--ACR-C
if BeardLib.Utils:ModLoaded("Remington ACR CQB") then
--Ranger plate
	self.parts.wpn_fps_ass_op68_mag_rangerplate.stats.reload = 20
	self.parts.wpn_fps_ass_op68_mag_rangerplate.stats.concealment = -2

--Milspec cosmetic
	self.parts.wpn_fps_ass_op68_stanag_30.stats = deep_clone(nostats)
	--Vintage mag
	self.parts.wpn_fps_ass_op68_stanag_20.stats = deep_clone(mag_66)
	self.parts.wpn_fps_ass_op68_stanag_20.stats.extra_ammo = -10
	end
--PM-63	
	if BeardLib.Utils:ModLoaded("PM-63 RAK") then
--Extended mag
	self.parts.wpn_fps_smg_pm63_m_ext.stats.reload = -20
	self.parts.wpn_fps_smg_pm63_m_ext.stats.extra_ammo = 10
	--Extended stock
	self.parts.wpn_fps_smg_pm63_st_ext.stats = self.parts.wpn_fps_smg_scorpion_s_unfolded.stats
--ROF higherer	
	self.parts.wpn_fps_smg_pm63_rof_937.custom_stats = {inf_rof_mult = 1.384615384615385}
	self.parts.wpn_fps_smg_pm63_rof_937.stats.spread = -10
	self.parts.wpn_fps_smg_pm63_rof_937.stats.damage = 0
	self.parts.wpn_fps_smg_pm63_rof_937.stats.recoil = 0
	end
	--SR1M
	if BeardLib.Utils:ModLoaded("SR1M") then
		--long slide
	self.parts.wpn_fps_pis_sr1_slide_long.stats.spread = 10
	self.parts.wpn_fps_pis_sr1_slide_long.stats.concealment = -5
	--silverslide
	self.parts.wpn_fps_pis_sr1_slide_silver.stats = deep_clone(nostats)
	--compensator
	self.parts.wpn_fps_pis_sr1_comp2.stats = 	self.parts.wpn_fps_upg_ass_ns_battle.stats
end

--AEK 919K
	if BeardLib.Utils:ModLoaded("AEK919K SMG") then
	self.parts.wpn_fps_smg_aek919_m_extended.stats = self.parts.wpn_fps_smg_pm63_m_ext.stats 
	self.parts.wpn_fps_smg_x_aek919_m_extended.extra_ammo = 20
	self.parts.wpn_fps_smg_aek919_s_unfold.stats = self.parts.wpn_fps_smg_scorpion_s_unfolded.stats
	self.parts.wpn_fps_smg_aek919_s_none.stats = {
		value = 0,
		recoil = -2,
		concealment = 2
	}
	self.parts.wpn_fps_smg_aek919_ns_exbarrel.stats = {
		value = 0,
		recoil = 5,
		concealment = -1
	}
	...
	--skorpion 9mm
	if BeardLib.Utils:ModLoaded("Scorpion Various Mag") then
self.parts.wpn_fps_smg_scorpion_m_straight.stats = {
		value = 0,
		reload = -12,
		concealment = -2,
		damage = 25
	...
	--M60 E4
	if BeardLib.Utils:ModLoaded("M60E4") then	
	self.parts.wpn_fps_upg_m60e4_barrel_long.stats.spread = 10
	self.parts.wpn_fps_upg_m60e4_barrel_long.stats.concealment = -5
	self.parts.wpn_fps_upg_m60e4_handguard_classic.stats = deep_clone(nostats)
	--Bipodd
	self.parts.wpn_fps_upg_m60e4_bipod.stats = deep_clone(nostats)
	end
	
	if BeardLib.Utils:ModLoaded("shatters_fury") then	
	--long barrel
	self.parts.wpn_fps_pis_shatters_fury_b_long.stats = {
		...
		self.parts.wpn_fps_pis_shatters_fury_g_ergo.stats = deep_clone(nostats)
		self.parts.wpn_fps_pis_shatters_fury_body_smooth.stats = deep_clone(nostats)
	end
	
	--Commando PDW stock for SCAR
	if self.parts.wpn_fps_ass_scar_s_pdw_collapsed then
	self.parts.wpn_fps_ass_scar_s_pdw_collapsed.stats = {
	concealment =5,
	recoil = - 15
	}	
	end
	if self.parts.wpn_fps_ass_scar_s_pdw then
	self.parts.wpn_fps_ass_scar_s_pdw.stats = {
	concealment =3,
	recoil = - 8
	}
	end
	
	--AK Various Attachments
	if BeardLib.Utils:ModLoaded("AK Various Attachment") then	
		self.parts.wpn_fps_ass_ak_b_gecko.stats = deep_clone(barrel_m1)
		self.parts.wpn_fps_ass_ak_fg_valme.stats = deep_clone(nostats)
		self.parts.wpn_fps_ass_ak_g_gecko.stats = deep_clone(nostats)
		self.parts.wpn_fps_ass_ak_s_gecko.stats = deep_clone(nostats)
		self.parts.wpn_fps_smg_akmsu_fg_mar.stats = deep_clone(nostats)
		self.parts.wpn_fps_smg_akmsu_s_rk62.stats = {
		concealment = 3,
		spread = -5,
		recoil = -10
		}
		...
		--AK Various Magz
	if BeardLib.Utils:ModLoaded("AK Various Mag") then	
			self.parts.wpn_fps_upg_ak_m_drum.stats = {
			concealment = -8,
			reload =-40,
			extra_ammo = 45
			}
		self.parts.wpn_fps_saiga_m_drum.stats = {
		concealment = -5,
		reload =-35,
		extra_ammo = 13
		}
		self.parts.wpn_fps_upg_ak_m_short.stats = {
		concealment = 3,
		extra_ammo = -10,
		reload = 30
		}
		...
		--AK17 VA
	if BeardLib.Utils:ModLoaded("AK17 Various Attachment") then
		self.parts.wpn_fps_ass_ak17_b_rpk.stats = deep_clone(barrel_m2)
		self.parts.wpn_fps_ass_ak17_b_16.stats = deep_clone(barrel_p2)
		self.parts.wpn_fps_ass_flint_fg_rpk.stats = deep_clone(nostats)
		self.parts.wpn_fps_ass_flint_s_fold.stats = {
		concealment= 2,
		recoil = -5
		}	
		self.parts.wpn_fps_ass_flint_m_308.stats = {
		concealment= 1,
		recoil = -15,
		spread = 5,
		damage = 80,
		ammo_pickup_min_mul = 0.4762,
		ammo_pickup_max_mul = 0.4496,
		extra_ammo = -10
		}
	end
			--ANPAS-13D Night Vision Sight
	if BeardLib.Utils:ModLoaded("ANPAS-13D Night Vision Sight") then
		self.parts.wpn_fps_upg_o_anpas13d.stats = {
		value = 2,
		zoom = 7,
		concealment = -3
		}
		self.parts.wpn_fps_upg_o_anpas13d.custom_stats = {disallow_ads_while_reloading = true}
end
		--Beretta Various Attachments
	if BeardLib.Utils:ModLoaded("Beretta Various Attachment") then 	
				Hooks:RemovePostHook("B9VariousModInit")
				self.parts.wpn_fps_pis_beretta_ns_vintage.stats = deep_clone(silstatsconc1)
				self.parts.wpn_fps_pis_beretta_co_equal.stats = self.parts.wpn_fps_upg_ass_ns_battle.stats
				self.parts.wpn_fps_pis_beretta_m_spike.stats = deep_clone(nostats)
				self.parts.wpn_fps_pis_beretta_body_sword.stats = deep_clone(nostats)
				self.parts.wpn_fps_pis_b93r_comp_93r.stats = self.parts.wpn_fps_upg_ass_ns_battle.stats
				...
				self.parts.wpn_fps_pis_b93r_vertgrip.override_weapon = {
					use_stance = "b93r",
					weapon_hold = "cobray"
					}
				self.wpn_fps_x_b92fs.override.wpn_fps_pis_b93r_vertgrip = {}
						--Revy Slide Override
	self.parts.wpn_fps_pis_beretta_body_sword.override = {
		wpn_fps_pis_beretta_sl_std = { unit = "units/mods/weapons/wpn_fps_pis_b92fsi_revy/wpn_fps_pis_beretta_sl_cutlass" },
		--Barrel Ext a_obj override-----------------------------------------------------------------------------------------
		wpn_fps_pis_beretta_co_co1 = { a_obj = "a_ns_revy" },
		wpn_fps_pis_beretta_co_co2 = { a_obj = "a_ns_revy" },
		wpn_fps_upg_ns_pis_large = { a_obj = "a_ns_revy" },
		wpn_fps_upg_ns_pis_medium = { a_obj = "a_ns_revy" },
		wpn_fps_upg_ns_pis_small = { a_obj = "a_ns_revy" },
		wpn_fps_upg_ns_pis_large_kac = { a_obj = "a_ns_revy" },
		wpn_fps_upg_ns_pis_medium_gem = { a_obj = "a_ns_revy" },
		wpn_fps_upg_ns_pis_medium_slim = { a_obj = "a_ns_revy" },
		wpn_fps_upg_pis_ns_flash = { a_obj = "a_ns_revy" },
		wpn_fps_upg_ns_pis_meatgrinder = { a_obj = "a_ns_revy" },
		wpn_fps_upg_ns_pis_ipsccomp = { a_obj = "a_ns_revy" },
		wpn_fps_upg_ns_ass_filter = { a_obj = "a_ns_revy" },
		wpn_fps_upg_ns_pis_jungle = { a_obj = "a_ns_revy" },
		--Custom Barrel Ext from this mod
		wpn_fps_pis_beretta_ns_vintage = { a_obj = "a_ns_revy" },
		wpn_fps_pis_beretta_co_vintage = { a_obj = "a_ns_revy" },
		wpn_fps_pis_beretta_co_equal = { a_obj = "a_ns_revy" },
		wpn_fps_pis_b93r_comp_93r = { a_obj = "a_ns_revy_93r" }, --positioned further on the tip of the barrel, to avoid the barrel 'eat' up the whole muzzle brake model
		--Front Sight override----------------------------------------------------------------------------------------------
		wpn_fps_pis_beretta_o_std = { unit = "units/mods/weapons/wpn_fps_pis_b92fsi_revy/ironsight_override/wpn_fps_pis_beretta_o_std_long" },
		wpn_upg_o_marksmansight_front = { a_obj = "a_os_revy" }
	}
	--PDTH Vintage Revy Slide workaround
	self.parts.wpn_fps_pis_beretta_ns_vintage.override = {
		wpn_fps_pis_beretta_co_co1 = {
			stats = {}
		}
	}
	end			
	Hooks:PostHook(WeaponTweakData, "init", "B9VariousModInit", function(self)	
	--CAP Section
	self:SetupAttachmentPoint( "b92fs", {
        name = "a_ns_revy",
        base_a_obj = "a_ns",
        position = Vector3( 0, 3.375, 0 ),
    })
	self:SetupAttachmentPoint( "b92fs", {
        name = "a_ns_revy_93r",
        base_a_obj = "a_ns",
        position = Vector3( 0, 6.375, 0 ),
    })
	self:SetupAttachmentPoint( "x_b92fs", {
        name = "a_ns_revy",
        base_a_obj = "a_ns",
        position = Vector3( 0, 3.375, 0 ),
    })
	...
	--Calico M9XX
			if BeardLib.Utils:ModLoaded("Calico SMG") then	
			self.parts.wpn_fps_smg_calico_body_carbine.stats = deep_clone(barrel_m1)
			self.parts.wpn_fps_smg_calico_body_full.stats = deep_clone(barrel_m2)
	...
	--M16 Carry Handle Rail
		if BeardLib.Utils:ModLoaded("Carry Handle Rail") then	
		self.parts.wpn_fps_upg_o_carrymount.stats = deep_clone(nostats)
		end
	--C-Mags For Various Weapons
		if BeardLib.Utils:ModLoaded("C-Mag for Various Weapon") then	
		Hooks:RemovePostHook("CMagVariousModInit")
			self.parts.wpn_fps_upg_ar15_m_cmag.stats ={
			concealment = -8,
			reload = -53,
			extra_ammo = 70
			}
			self.wpn_fps_ass_m16.override.wpn_fps_upg_ar15_m_cmag = {
		stats = {
			concealment = -5, reload = -53, extra_ammo = 80
		}}
		self.wpn_fps_smg_olympic.override.wpn_fps_upg_ar15_m_cmag = {
		stats = {
			concealment = -5, reload = -53, extra_ammo = 80
		}}
			self.parts.wpn_fps_upg_g36_m_cmag.stats ={
			concealment = -5,
			reload = -53,
			extra_ammo = 70
			}
	...
	--CAP Section
Hooks:PostHook(WeaponTweakData, "init", "CMagVariousModInit", function(self)

	--CAP Section
	self:SetupAttachmentPoint( "g36", {
        name = "a_g36_m_drum",
		part_attach_data = {
			{"wpn_fps_upg_g36_m_dummy"}, 
			"g_mag"
		},
...
--Colt Carry Handle Optics
	if BeardLib.Utils:ModLoaded("Colt Optic Carry") then	
self.parts.wpn_fps_upg_o_coltcarry.stats = {
		value = 1,
		zoom = 5,
		concealment = -1}		
	self.parts.wpn_fps_upg_o_coltcarry.custom_stats = {disallow_ads_while_reloading = true}
	end
--Crosskill Laser Gripppppp
	if BeardLib.Utils:ModLoaded("Crosskill Laser Grip") then	
		self.parts.wpn_fps_pis_1911_g_laser.stats = deep_clone(nostats)
	end
--CZ Skorpion Evo
	if BeardLib.Utils:ModLoaded("CZ Scorpion Evo") then	
		...
		self.parts.wpn_fps_smg_czevo_barrel_silenced.stats = deep_clone(silstatsconc1)
		self.parts.wpn_fps_smg_czevo_a_strong.stats = deep_clone(nostats)
		self.parts.wpn_fps_smg_czevo_a_classic.stats = deep_clone(nostats)
		self.parts.wpn_fps_smg_czevo_mag_speed.stats = deep_clone(nostats)
	end
--ELCAN C79
	if BeardLib.Utils:ModLoaded("ELCAN C79") then	
		self.parts.wpn_fps_upg_o_c79.custom_stats = {disallow_ads_while_reloading = true}
		self.parts.wpn_fps_upg_o_c79.stats = 	self.parts.wpn_fps_upg_o_specter.stats
	end
--F2000 Telescopic Sight, the proper sight
	if BeardLib.Utils:ModLoaded("F2000 Telescopic Sight") then	
		self.parts.wpn_fps_ass_komodo_o_tele.custom_stats = {disallow_ads_while_reloading = true}
...
--FAMAS Various Attachment
	if BeardLib.Utils:ModLoaded("FAMAS Various Attachment") then	
		self.parts.wpn_fps_ass_famas_body_feline.stats = deep_clone(nostats)
		self.parts.wpn_fps_ass_famas_m_dual.stats = { concealment = -2, reload = 65}
	end
--Fightlite Raider
	if BeardLib.Utils:ModLoaded("SCR Raider") then	
		self.parts.wpn_fps_pis_raider_st_vector.stats = {
		concealment = -2, 
		recoil = 5,
		spread = 2}
		end
--Full auto conversion pack
	if BeardLib.Utils:ModLoaded("Full auto conversion pack") then	
		self.parts.wpn_fps_upg_b92fs_autofire.custom_stats = {inf_rof_mult= 1.349325337331334}
		self.parts.wpn_fps_upg_b92fs_autofire.stats = {}
		self.parts.wpn_fps_upg_hs2000_autofire.custom_stats = {inf_rof_mult= 2}
		self.parts.wpn_fps_upg_hs2000_autofire.stats = {}self.parts.wpn_fps_upg_m1911_autofire.custom_stats = {inf_rof_mult= 1.6667}
		self.parts.wpn_fps_upg_m1911_autofire.stats = {}self.parts.wpn_fps_upg_glock_autofire.custom_stats = {inf_rof_mult= 2.1}
		self.parts.wpn_fps_upg_glock_autofire.stats = {}
		self.parts.wpn_fps_upg_xm25_autofire.stats = {}
		self.parts.wpn_fps_upg_spas12_autofire.stats = {}
		self.parts.wpn_fps_upg_m1014_autofire.stats = {}
	end
--G3 Various Attachment
	if BeardLib.Utils:ModLoaded("G3 Various Attachment") then	
		self.parts.wpn_fps_ass_g3_fg_carbine.stats = {}
		self.parts.wpn_fps_lmg_hk21_fg_mp5.stats = deep_clone(barrel_p3)
		self.parts.wpn_fps_ass_g3_s_polymer.stats = {}
		self.parts.wpn_fps_ass_g3_m_50drum.stats = {
		concealment= -4,
		reload = -35,
		extra_ammo = 30}
		self.parts.wpn_fps_ass_g3_s_retractable.stats = self.parts.wpn_fps_smg_mp5_s_adjust.stats
		self.parts.wpn_fps_lmg_hk21_s_low.stats = self.parts.wpn_fps_smg_mp5_s_adjust.stats
...
--G18c Drum
	if self.parts.wpn_fps_pis_g18c_drum then
		Hooks:RemovePostHook("G18DrumMagModInit")
		 self.parts.wpn_fps_pis_g18c_drum.stats = {
		 concealment= -4,
		 extra_ammo= 31,
		reload = -45
		 }		

		self.wpn_fps_pis_x_g18c.override.wpn_fps_pis_g18c_drum = {
		  animations = {
		...
		stats = {
			concealment = -4, reload = -45, extra_ammo = 62, recoil = 8
		}}
	end
--Galil Various Attachment
	if BeardLib.Utils:ModLoaded("Galil Various Attachment") then	
		self.parts.wpn_fps_ass_galil_m_drum.stats = { concealment = -5, extra_ammo = 45, reload = -53}
		self.parts.wpn_fps_ass_galil_o_galatz.stats = { concealment = -2, zoom = 7}
		self.parts.wpn_fps_ass_galil_m_35r.stats = { concealment = -2, extra_ammo = 10, reload = -15}
		self.parts.wpn_fps_ass_galil_m_762c.stats = { concealment = 1, extra_ammo = -5, damage = 45, recoil= -10, spread = 5}
	end
--Gatmalite
	if BeardLib.Utils:ModLoaded("gatmalite") then	
		Hooks:RemovePostHook("gatmaliteInit")
		self.parts.wpn_fps_m4_uupg_b_gatmalite.forbids = {
		...
		self.parts.wpn_fps_m4_uupg_b_gatmalite.has_description = true
		self.parts.wpn_fps_m4_uupg_b_gatmalite.custom_stats = {inf_rof_mult = 4.375}
		self.parts.wpn_fps_m4_uupg_b_gatmalite.stats = { concealment = -2, spread = -20, spread_moving= -10, suppression = -12, recoil= -5}
	end
--Glock Beta C Mag		
	if BeardLib.Utils:ModLoaded("Glock Beta-C mag") then	
	Hooks:RemovePostHook("glockbetacmagmodInit")
		self.parts.wpn_fps_pis_g18c_m_beta.stats = {concealment= - 6, reload = -53, extra_ammo = 0}
Hooks:PostHook( WeaponFactoryTweakData, "init", "glockcmagmodInit", function(self)
	if not self.wpn_fps_smg_x_polymer.override then
		self.wpn_fps_smg_x_polymer.override = {}
	end
	if not self.wpn_fps_smg_polymer.override then
		self.wpn_fps_smg_polymer.override = {}
	end

	self.parts.wpn_fps_pis_g18c_m_beta.override_weapon = {
		CLIP_AMMO_MAX = 100,
	}	
	self.wpn_fps_pis_x_g17.override.wpn_fps_pis_g18c_drum_up = {
		animations = {
				reload_not_empty = "reload_not_empty_right",
				reload = "reload_right",
				reload_left = "reload_left",
	...
	self.wpn_fps_jowi.override.wpn_fps_pis_g18c_drum_up = {
		animations = {
				reload_not_empty = "reload_not_empty_right",
				reload = "reload_right",
				reload_left = "reload_left",
	...
	self.wpn_fps_pis_x_g22c.override.wpn_fps_pis_g18c_drum_up = {
		animations = {
				reload_not_empty = "reload_not_empty_right",
				reload = "reload_right",
				reload_left = "reload_left",
	...
	self.wpn_fps_smg_polymer.override.wpn_fps_pis_g18c_m_beta = {
		adds = {"wpn_fps_smg_polymer_m_dummy","wpn_fps_pis_g18c_drum_up"}
	}
	self.wpn_fps_smg_x_polymer.override.wpn_fps_pis_g18c_m_beta = {
		adds = {"wpn_fps_smg_polymer_m_dummy","wpn_fps_pis_g18c_drum_up"},
		override_weapon = {CLIP_AMMO_MAX = 200, weapon_hold = "x_coal", use_stance = "x_coal",
		    animations = {
			    reload_name_id = "x_coal"
		    }
		}
	}
	self.wpn_fps_smg_polymer.override.wpn_fps_pis_g18c_drum_up = {
		animations = {},
		a_obj= "a_drum_up"
	}
    self.wpn_fps_smg_x_polymer.override.wpn_fps_pis_g18c_drum_up = {
		animations = {},
		a_obj= "a_drum_up"
	}
    self.wpn_fps_pis_x_g18c.override.wpn_fps_pis_g18c_drum_up = {
		animations = {
				reload_not_empty = "reload_not_empty_right",
				reload = "reload_right",
				reload_left = "reload_left",
    ...
    self.wpn_fps_pis_x_g18c.override.wpn_fps_pis_g18c_m_beta = {
		override_weapon = {CLIP_AMMO_MAX = 200}}
				
	self.wpn_fps_pis_x_g22c.override.wpn_fps_pis_g18c_m_beta = {
		override_weapon = {CLIP_AMMO_MAX = 200, AMMO_MAX = 200}}

	self.wpn_fps_smg_x_polymer.override.wpn_fps_pis_g18c_m_beta = {
		override_weapon = {CLIP_AMMO_MAX = 200}}
		
	self.wpn_fps_pis_x_g17.override.wpn_fps_pis_g18c_m_beta = {
		override_weapon = {CLIP_AMMO_MAX = 200, AMMO_MAX = 200}}

	self.wpn_fps_jowi.override.wpn_fps_pis_g18c_m_beta = {
		override_weapon = {CLIP_AMMO_MAX = 200, AMMO_MAX = 200}}
	end )
...
--L85 Various Attachment
	if BeardLib.Utils:ModLoaded("L85 Various Attachment") then	
		self.parts.wpn_fps_upg_l85a2_bipod.stats = deep_clone(nostats)
		self.parts.wpn_fps_ass_l85a2_vg_back.stats = deep_clone(nostats)
		self.parts.wpn_fps_ass_l85a2_fg_flush.stats = {concealment = 2, recoil= -5, spread = -3}
		self.parts.wpn_fps_ass_l85a2_b_heavy.stats = deep_clone(barrel_m2)
		self.parts.wpn_fps_ass_l85a2_m_drum.stats = { concealment = -5, extra_ammo = 50}
	end
--M16 LSW Handguard
	if BeardLib.Utils:ModLoaded("M16 LSW Handguard") then	
		self.parts.wpn_fps_ass_m16_fg_lsw.stats = deep_clone(nostats)
	end
--	M1911 Attachments Pack
	if BeardLib.Utils:ModLoaded("M1911 Attachments Pack") then	
		self.parts.wpn_fps_pis_m1911_ns_cutts.stats ={ concealment= -1, recoil = 5, spread = 4}
		self.parts.wpn_fps_pis_m1911_s_skeletal.stats ={ concealment= -2, recoil = 6}
		self.parts.wpn_fps_pis_m1911_fg_custom.stats ={ concealment= -1, recoil = 6}
		self.parts.wpn_fps_pis_m1911_fg_tommy.stats ={ concealment= -3, recoil = 10}
		self.parts.wpn_fps_pis_m1911_s_wooden.stats ={ concealment= -4, recoil = 10}
		self.parts.wpn_fps_pis_1911_m_banana.stats ={ concealment= -8, extra_ammo = 23, recoil = -8, reload = -60}
		self.wpn_fps_x_1911.override.wpn_fps_pis_1911_m_banana = self.wpn_fps_x_1911.override.wpn_fps_pis_1911_m_banana or {}
		self.wpn_fps_x_1911.override.wpn_fps_pis_1911_m_banana.stats = deep_clone(self.parts.wpn_fps_pis_1911_m_banana.stats)
		self.wpn_fps_x_1911.override.wpn_fps_pis_1911_m_banana.stats.extra_ammo = self.wpn_fps_x_1911.override.wpn_fps_pis_1911_m_banana.stats.extra_ammo * 2
	end
--MAC-10 Various Attachment
	if BeardLib.Utils:ModLoaded("MAC-10 Various Attachment") then	
		Hooks:RemovePostHook("MAC10VariousModInit")

		self.parts.wpn_fps_smg_mac10_ns_blade.stats ={ concealment= -2, damage = 5, recoil = -12, spread = 2}
		self.parts.wpn_fps_smg_mac10_s_nil.stats ={ concealment= 3, recoil = -12}
		self.parts.wpn_fps_smg_mac10_anim_suppgrip.stats ={ concealment= -2, recoil = 5, spread = 2}
		self.parts.wpn_fps_smg_mac10_m_casket.stats ={ concealment= -3, recoil = -5, extra_ammo=35, reload= -30}
		self.parts.wpn_fps_smg_mac10_s_stowed.stats ={ concealment= 1, recoil = -2}
		self.parts.wpn_fps_smg_mac10_ns_ghetto.stats ={ concealment=-4, recoil = 10, spread = 10, suppression = -6}
		Hooks:PostHook( PlayerTweakData, "init", "MAC10VariousModIn", function(self)
	
	self.stances.mac10_strap = deep_clone(self.stances.mac10)
	self.stances.mac10_strap.steelsight.shoulders.translation = Vector3(-9.3, 0.646, 4.637)
	-- self.stances.mac10_strap.standard.shoulders.translation = self.stances.mac10_strap.standard.shoulders.translation + standard_translation

	self.stances.mac10_suppgrip = deep_clone(self.stances.mac10)
	self.stances.mac10_suppgrip.steelsight.shoulders.translation = Vector3(-10.781, 2.646, 5.637)
	-- self.stances.mac10_suppgrip.standard.shoulders.translation = self.stances.mac10_suppgrip.standard.shoulders.translation + standard_translation

	end )
Hooks:PostHook(WeaponTweakData, "init", "MAC10VariousModIn", function(self)
		
	self:SetupAttachmentPoint( "mac10", {
        name = "a_fl_ghetto",
        base_a_obj = "a_fl",
        position = Vector3( -0.25, 7.5, 2 )
    })

	end)
Hooks:PostHook( WeaponFactoryTweakData, "init", "MAC10VariousModIn", function(self)
	

	self.parts.wpn_fps_smg_mac10_ns_ghetto.forbids = { "wpn_fps_addon_ris" }
	self.parts.wpn_fps_smg_mac10_ns_ghetto.override = { 
		wpn_fps_upg_fl_ass_smg_sho_surefire = { a_obj = "a_fl_ghetto" },
		wpn_fps_upg_fl_ass_smg_sho_peqbox = { a_obj = "a_fl_ghetto" },
		wpn_fps_upg_fl_ass_laser = { a_obj = "a_fl_ghetto" },
		wpn_fps_upg_fl_ass_peq15 = { a_obj = "a_fl_ghetto" },
		wpn_fps_upg_fl_ass_utg = { a_obj = "a_fl_ghetto" }
	}
	
	self.parts.wpn_fps_smg_mac10_anim_suppgrip.requires = { "wpn_fps_upg_ns_ass_smg_large", "wpn_fps_smg_cobray_ns_silencer", "wpn_fps_smg_cobray_ns_barrelextension", "wpn_fps_smg_mac10_ns_ghetto" }
	self.parts.wpn_fps_smg_mac10_anim_suppgrip.override_weapon = {
		--[[ shepheard = best || ak47/m4 = similar to shepheard, but imo jumps too much || mp5 = reload transition jumps far more than ak47/m4 ]]
		use_stance = "mac10_suppgrip",
		weapon_hold = "shepheard" 
	}
	
	self.parts.wpn_fps_smg_mac10_vg_strap.override_weapon = {
		use_stance = "mac10_strap", --[[mac10_strap]]
		weapon_hold = "cobray",
		kick = {standing = {
			-0.6,
			0.6,
	...
	--vg and suppgrip arent compatible with each other
	self:double_forbid("wpn_fps_smg_mac10_vg_strap", "wpn_fps_smg_mac10_anim_suppgrip")
	
	
	--carl was here xd	
	self:double_forbid("wpn_fps_smg_mac10_vg_strap", "wpn_fps_smg_mac10_ns_blade")
--	self.parts.wpn_fps_smg_mac10_ns_blade.forbids = { "wpn_fps_smg_mac10_body_ris" }
	
	self.parts.wpn_fps_smg_mac10_ns_blade.override.wpn_fps_smg_mac10_body_ris = {
		unit = "units/mods/weapons/wpn_fps_smg_mac10_pts/wpn_fps_smg_mac10_body_ris_alt"
	}
	
	self.parts.wpn_fps_smg_mac10_body_modern.override = self.parts.wpn_fps_smg_mac10_body_modern.override or {}	
	self.parts.wpn_fps_smg_mac10_body_modern.override.wpn_fps_smg_mac10_ns_blade = {
		unit = "units/mods/weapons/wpn_fps_smg_mac10_pts/wpn_fps_smg_mac10_ns_blade_alter"
	}
...
--MARS Sight
	if BeardLib.Utils:ModLoaded("MARS Sight") then	
		self.parts.wpn_fps_ass_komodo_o_mars.stats ={ concealment = -1, zoom = 3}
		end
	--Project UAE
	if BeardLib.Utils:ModLoaded("Useless Attachment Expansion") then
self.parts.wpn_fps_upg_i_brrt.stats = {
		value = 0,
		inf_rof_mult= 2,
		spread= -10
	}
	self.parts.wpn_fps_pis_beretta_b_suppressed.stats = deep_clone{silstatsconc0}
	self.parts.wpn_fps_pis_rage_b_suppressed.stats = deep_clone{silstatsconc0}
	self.parts.wpn_fps_ass_akm_b_bling.stats = deep_clone{nostats}
	self.parts.wpn_fps_pis_b_int.stats = deep_clone{silstatsconc1}
	self.parts.wpn_fps_pis_rage_b_pocket.stats = deep_clone{barrel_p3}
	self.parts.wpn_fps_pis_rage_b_velocity.stats = deep_clone{barrel_m1}
	self.parts.wpn_fps_pis_b_pocket.stats = deep_clone{barrel_p3}
	self.parts.wpn_fps_pis_beretta_b_heavy.stats = deep_clone{barrel_m1}
	self.parts.wpn_fps_pis_b_enough.stats = deep_clone{barrel_p1}
	self.parts.wpn_fps_smg_b_long.stats = deep_clone{barrel_m1}
	self.parts.wpn_fps_upg_b_heavy.stats = deep_clone{barrel_m1}
	self.parts.wpn_fps_sho_ben_b_sup.stats = deep_clone{silstatsconc2}
	self.parts.wpn_fps_upg_b_desertops.stats = deep_clone{silstatsconc0}
	self.parts.wpn_fps_upg_b_bucket.stats = deep_clone{silstatsconc0}
	self.parts.wpn_fps_lmg_rpk_b_cqc.stats = deep_clone{barrel_p3}
	self.parts.wpn_fps_pis_rage_sl_diablo.stats = deep_clone{barrel_m2}
	self.parts.wpn_fps_pis_peacemaker_sl_spitfire.stats = deep_clone{barrel_m3}
	self.parts.wpn_fps_ass_akm_fg_bling.stats = deep_clone{nostats}
	self.parts.wpn_fps_ass_akm_fg_bling.stats = deep_clone{nostats}
	self.parts.wpn_fps_ass_s552_fg_rubber.stats = deep_clone{nostats}
	self.parts.wpn_fps_ass_galil_fg_rubber.stats = deep_clone{nostats}
	self.parts.wpn_fps_ass_fal_fg_rubber.stats = deep_clone{nostats}
	self.parts.wpn_fps_ass_amcar_fg_rubber.stats = deep_clone{nostats}
	self.parts.wpn_fps_ass_fg_rubber.stats = deep_clone{nostats}
	self.parts.wpn_fps_sho_fg_rubber.stats = deep_clone{nostats}
	self.parts.wpn_fps_smg_x_uzi_fg_krink.stats = deep_clone{nostats}
	self.parts.wpn_fps_upg_fg_coal.stats = deep_clone{nostats}
	self.parts.wpn_fps_pis_ma_light.stats = deep_clone{nostats} -- remove, silly
	self.parts.wpn_fps_pis_ma_light.pcs = nil
	self.parts.wpn_fps_pis_ma_stacked.stats = deep_clone{nostats} -- remove, silleh
	self.parts.wpn_fps_pis_ma_stacked.pcs = nil
	self.parts.wpn_fps_ass_m_steel.stats = deep_clone{nostats}
	self.parts.wpn_fps_ass_akm_m_bling.stats = deep_clone{nostats}
	self.parts.wpn_fps_lmg_rpk_m_breech.stats = deep_clone{nostats} -- remove, quite silly
	self.parts.wpn_fps_ass_m_saiga.stats = deep_clone{nostats} --remove, silly
	self.parts.wpn_fps_pis_glock_m_press1.stats = deep_clone{nostats} --remove, silly and quite disgusting
	self.parts.wpn_fps_pis_glock_m_press1.pcs = nil
	self.parts.wpn_fps_pis_glock_m_press2.stats = deep_clone{nostats} --remove, silly and quite disgusting
	self.parts.wpn_fps_pis_glock_m_press2.pcs = nil
	self.parts.wpn_fps_pis_m_milspec.stats = deep_clone{nostats} --remove, silly and quite disgusting
	self.parts.wpn_fps_pis_m_tac.stats = deep_clone{nostats} 
	self.parts.wpn_fps_glock_m_flatplate.stats = {extra_ammo = -1, concealment = 1}
	self.parts.wpn_fps_pis_body_slim1.stats = {} --remove, mega silly and absolutely disgusting
	self.parts.wpn_fps_pis_body_slim2.stats = {} --remove, mega silly and absolutely disgusting
	self.parts.wpn_fps_upg_ga_up.stats = {} --remove, very silly and absolutely atrocious
	self.parts.wpn_fps_upg_ga_ergo.stats = {} --remove, very silly and absolutely atrocious
	self.parts.wpn_fps_pis_judge_g_wood.stats = deep_clone{nostats} 
	self.parts.wpn_fps_pis_judge_g_rubber.stats = deep_clone{nostats} 
	self.parts.wpn_fps_pis_judge_g_sharps.stats = deep_clone{nostats} 
	self.parts.wpn_fps_pis_judge_g_sport.stats = deep_clone{nostats} 
	self.parts.wpn_fps_pis_ppk_g_ergo.stats = deep_clone{nostats} 
	self.parts.wpn_fps_ass_scar_g_std.stats = deep_clone{nostats} 
	self.parts.wpn_fps_pis_g17_g_ergo.stats = deep_clone{nostats} 
	self.parts.wpn_fps_pis_usp_g_ergo.stats = deep_clone{nostats} 
	self.parts.wpn_fps_pis_breech_g_ergo.stats = deep_clone{nostats} 
	self.parts.wpn_fps_upg_g_ergo.stats = deep_clone{nostats} 
	self.parts.wpn_fps_upg_g_flint.stats = deep_clone{nostats} 
	self.parts.wpn_fps_pis_2006m_g_comp.stats = {concealment = 3, spread = -5, recoil= -4}
	self.parts.wpn_fps_pis_1911_g_short.stats = {concealment = 2, spread = -5}
	self.parts.wpn_fps_pis_1911_g_smooth.stats = {concealment = 2, spread = -2, recoil= -4}
	self.parts.wpn_fps_upg_g_coal.stats = {concealment = 2, spread = -4, recoil= -4}
	self.parts.wpn_fps_pis_ns_rec.stats = {concealment = 2, spread = -4, recoil= -4}
	self.parts.wpn_fps_pis_ns_block.stats = {concealment = 2, spread = -4, recoil= -4}
	self.parts.wpn_fps_pis_ns_acc.stats = {concealment = 2, spread = -4, recoil= -4}
	self.parts.wpn_fps_pis_ns_stub.stats = {concealment = 2, spread = -4, recoil= -4}
	self.parts.wpn_fps_pis_ns_agr.stats = deep_clone{nostats} 
	self.parts.wpn_fps_pis_ns_dmg.stats = deep_clone{nostats} 
	self.parts.wpn_fps_pis_ns_baka.stats = deep_clone{nostats} 
	self.parts.wpn_fps_pis_s_wire.stats = { recoil = 5, concealment = -2} 
	self.parts.wpn_fps_ass_akm_s_bling.stats = deep_clone{nostats} 
	self.parts.wpn_fps_upg_s_adjust.stats =  self.parts.wpn_fps_smg_mp5_s_adjust.stats
	self.parts.wpn_fps_x_s_wire.stats =  deep_clone{nostats} 
	self.parts.wpn_fps_smg_s_wire.stats =  {concealment= 2, recoil= -5}
	self.parts.wpn_fps_ass_s_tantal.stats =  {concealment= 4, recoil= -10, spread = -3}
	self.parts.wpn_fps_smg_s_rus.stats =  deep_clone(nostats)
	self.parts.wpn_fps_smg_coal_s_sniper.stats =  deep_clone(nostats)
	self.parts.wpn_fps_upg_ak_s_sharps.stats =  deep_clone(nostats)
	self.parts.wpn_fps_smg_s_plank.stats =  deep_clone(nostats)
	self.parts.wpn_fps_sho_striker_ck_shellhose.stats =  deep_clone(nostats)
	self.parts.wpn_fps_upg_s_coal.stats =  {concealment= 4, recoil= -10, spread = -3}
	end
--Tec9drum
	if BeardLib.Utils:ModLoaded("Tec9drum") then	
		self.parts.wpn_fps_smg_tec9_m_drum.stats =  {extra_ammo=30, reload = -45, concealment = -4}
	end			
--AUG A3 9mm XS
	if BeardLib.Utils:ModLoaded("AUG A3 9mm XS") then	
		self.parts.wpn_fps_upg_aug9mm_barrel_long.stats =  deep_clone(barrel_m2)
		self.parts.wpn_fps_upg_aug9mm_barrel_med.stats =  deep_clone(barrel_m1)
		self.parts.wpn_fps_upg_aug9mm_stock_stable.stats = {concealment= - 1, recoil = 4}
		self.parts.wpn_fps_upg_aug9mm_stock_tactical.stats = {concealment= - 1, recoil = 4}
		self.parts.wpn_fps_upg_aug9mm_stock2.stats = {concealment= 1, recoil = -2}
		self.parts.wpn_fps_upg_aug9mm_stock2_stable.stats = {spread= 3, recoil = -5}
	end
	
--L22A2
	if BeardLib.Utils:ModLoaded("Enfield L22") then	
		self.parts.wpn_fps_pdw_fckmyfingers_b_medium.stats =  deep_clone(barrel_m1)
		self.parts.wpn_fps_pdw_fckmyfingers_m_short.stats =  deep_clone(barrel_p1)
		self.parts.wpn_fps_pdw_fckmyfingers_o_susat.stats =  self.parts.wpn_fps_upg_o_specter.stats
		self.parts.wpn_fps_pdw_fckmyfingers_o_susat.custom_stats = {disallow_ads_while_reloading = true}

	end
--Luger Artillery
	if BeardLib.Utils:ModLoaded("Enfield L22") then	
	Hooks:RemovePostHook {"ArtyLugerModInit"}
	self.parts.wpn_fps_pis_breech_m_drum.stats = {concealment = -4, extra_ammo= 24, reload = -40}
	self.parts.wpn_fps_pis_breech_s_holster.stats = {concealment = -3, recoil = 13}
	self.parts.wpn_fps_pis_breech_b_arty.stats = {concealment = -3, recoil = 5 , spread = 10}
		end
--SOCOM-23 LAM
	if BeardLib.Utils:ModLoaded("SOCOM-23 LAM gadget") then
		self.parts.wpn_fps_upg_fl_pis_LAM.stats = {concealment = -1, spread = 5}
	end
--Belomo sight
	if BeardLib.Utils:ModLoaded("BelOMO PK-01 VI") then
		self.parts.wpn_fps_upg_o_pk01_vi.stats = self.parts.wpn_fps_upg_o_cs.stats
	end
 --X-15 Drum Mag
 	if BeardLib.Utils:ModLoaded("X-15 50 Round Drum Magazine") then

	self.parts.wpn_fps_upg_m4_m_x15drum.stats = { concealment = -4, reload = -45}
	self.parts.wpn_fps_upg_m4_m_x15drum.override_weapon = {
		CLIP_AMMO_MAX = 50}
 end
 --MP5 Drum Mag
  	if BeardLib.Utils:ModLoaded("compact5drum") then

	self.parts.wpn_fps_smg_m_mp5_drum.stats = { concealment = -4, reload = -45}
	self.parts.wpn_fps_smg_m_mp5_drum.override_weapon = {
		CLIP_AMMO_MAX = 50 }
		self.parts.wpn_fps_smg_m_mp5_drum_c.stats = { concealment = -8, reload = -53}
	self.parts.wpn_fps_smg_m_mp5_drum_c.override_weapon = {
		CLIP_AMMO_MAX = 100 }
		self.parts.wpn_fps_smg_x_m_mp5_drum.stats = { concealment = -4, reload = -45}
	self.parts.wpn_fps_smg_x_m_mp5_drum.override_weapon = {
		CLIP_AMMO_MAX = 100	}
		end
--Banana magz for AK
  	if BeardLib.Utils:ModLoaded("banmag") then
Hooks:RemovePostHook {"banmagInit"}
		self.parts.wpn_fps_upg_banana_mag.stats = { concealment = -2, reload = -20}
	self.parts.wpn_fps_upg_banana_mag.override_weapon = {
		CLIP_AMMO_MAX = 45 }
		end
--M249 Backpack mag
  	if BeardLib.Utils:ModLoaded("M249 SAW Ammo Backpack") then

		self.parts.wpn_fps_lmg_m249_m_ammo.custom_stats = { movement_speed = 0.55}
		self.parts.wpn_fps_lmg_m249_m_ammo.stats = { concealment = -20, reload = -80}
	self.parts.wpn_fps_lmg_m249_m_ammo.override_weapon = {
		CLIP_AMMO_MAX = 800, AMMO_MAX = 800 }
		end

--MGL Proprietary Sight
	if BeardLib.Utils:ModLoaded("MGL Proprietary Sight") then	
		self.parts.wpn_fps_gre_m32_o_leaf.stat = {gadget_zoom = 2, concealment = -2}
		self.parts.wpn_fps_gre_m32_o_slant.stat = {spread = 4, concealment = -3}
	end
--FAL Various Attachment
	if BeardLib.Utils:ModLoaded("FAL Various Attachment") then	
		Hooks:RemovePostHook {"FALVariousmodInit"}
		
	Hooks:PostHook( WeaponFactoryTweakData, "init", "FALVariousmodAdaptation", function(self)

	table.insert(self.wpn_fps_ass_fal.default_blueprint, "wpn_fps_ass_fal_ns_standard")
	table.insert(self.wpn_fps_ass_fal_npc.default_blueprint, "wpn_fps_ass_fal_ns_standard")
	table.insert(self.wpn_fps_ass_fal.uses_parts, "wpn_fps_ass_fal_ns_standard")
	table.insert(self.wpn_fps_ass_fal_npc.uses_parts, "wpn_fps_ass_fal_ns_standard")
	
	
	--[[Barrel Ext FAL behaviour]]--------------------------------------------------------------------------------------------------------------------------------
	local moved_barrel_ext = {        
		"wpn_fps_upg_ns_ass_smg_large",
		"wpn_fps_upg_ns_ass_smg_medium",
		"wpn_fps_upg_ns_ass_smg_small",
		"wpn_fps_upg_ns_ass_smg_firepig",
	...
	--MW2 fg barrel_ext alignment
	for i, part_id in ipairs(moved_barrel_ext) do
        self.parts.wpn_fps_ass_fal_fg_stg58.override[part_id] = { a_obj = "a_ns_std" }
    end 

	--CQB Foregrip
	self.parts.wpn_fps_ass_fal_fg_01.override = self.parts.wpn_fps_ass_fal_fg_01.override or {}
	self.parts.wpn_fps_ass_fal_fg_01.override.wpn_fps_ass_fal_ns_standard = { unit = "units/mods/weapons/wpn_fps_ass_fal_muzzle/wpn_fps_ass_fal_ns_cqb" }

	--Standard & Wooden Foregrip
	self.parts.wpn_fps_ass_fal_fg_standard.override = self.parts.wpn_fps_ass_fal_fg_standard.override or {}
	self.parts.wpn_fps_ass_fal_fg_wood.override = self.parts.wpn_fps_ass_fal_fg_wood.override or {}
	for j, part_id in ipairs(moved_barrel_ext) do
	...
	--Romat Foregrip
	self.parts.wpn_fps_ass_fal_fg_03.override = self.parts.wpn_fps_ass_fal_fg_03.override or {}
	self.parts.wpn_fps_ass_fal_fg_03.override.wpn_fps_ass_fal_ns_standard = { unit = "units/mods/weapons/wpn_fps_ass_fal_muzzle/wpn_fps_ass_fal_ns_long" }
	for k, part_id in ipairs(moved_barrel_ext) do    
...
Hooks:PostHook( WeaponTweakData, "init", "FALVariousmodAdaptation", function(self)
...
self.parts.wpn_fps_ass_fal_fg_stg58.stats = deep_clone (barrel_p1)
self.parts.wpn_fps_ass_fal_s_modern.stats = deep_clone (nostats)
self.parts.wpn_fps_ass_fal_fg_peru.stats = deep_clone (nostats)
self.parts.wpn_fps_ass_fal_fg_04.stats = deep_clone (nostats)
end
--SCAR Various Attachment
	if BeardLib.Utils:ModLoaded("SCAR Various Attachment") then	
	self.parts.wpn_fps_ass_scar_s_tactical.stats = {recoil = -5, concealment = 2}
	self.parts.wpn_fps_ass_scar_m_full.stats = {extra_ammo = 20, concealment = -4, reload = -40}
	self.parts.wpn_fps_ass_scar_s_skel.stats = self.parts.wpn_fps_ass_scar_s_pdw.stats
	end

--Kriss Vector Proto Stock	
	if BeardLib.Utils:ModLoaded("Prototype Stock") then	
	self.parts.wpn_fps_smg_polymer_stock_prototype.stats = {concealment = 2, recoil= -5}
	end
--Luger Astra
	if BeardLib.Utils:ModLoaded("Luger Astra") then	
		self.parts.wpn_fps_upg_luger_autofire.stats = {spread = -5, recoil = -3}
	end
--US Optics ST-10
	if BeardLib.Utils:ModLoaded("US Optics ST-10") then	
	self.parts.wpn_fps_upg_o_st10.stats = {concealment = -3, zoom = 8}
	end
--Kemper XL Sight
	if BeardLib.Utils:ModLoaded("Kemper XL Sight") then	
	self.parts.wpn_fps_upg_o_kemper.stats = {concealment= -1, zoom= 0}
end
--Mepro M21
	if BeardLib.Utils:ModLoaded("Mepro M21") then	
	self.parts.wpn_fps_upg_o_mepro.stats = {concealment= -1, zoom= 0}
end
--SUSAT
	if BeardLib.Utils:ModLoaded("SUSAT`") then	
	Hooks:RemovePostHook ("SUSATModInit")
	Hooks:PostHook(WeaponFactoryTweakData, "init", "SUSATModINF", function(self)
	local custom_sight_id = "wpn_fps_upg_o_susat"
	local based_on_id = "wpn_fps_upg_o_acog"
	local offset = Vector3(0, 0, -1.409822)
	...
	for _, part_id in pairs(self.parts) do
		...
		self.parts.wpn_fps_upg_o_susat.custom_stats = {disallow_ads_while_reloading = true}
		self.parts.wpn_fps_upg_o_susat.stats = {concealment= -2, zoom= 5}

end
--Browning High Power
 if BeardLib.Utils:ModLoaded("hpb") then
  self.parts.wpn_fps_pis_hpb_g_black.stats=deep_clone(nostats)
  self.parts.wpn_fps_pis_hpb_g_white.stats=deep_clone(nostats)
  self.parts.wpn_fps_pis_hpb_g_cherry.stats=deep_clone(nostats)
  self.parts.wpn_fps_pis_hpb_thread.stats=deep_clone(nostats)
  self.parts.wpn_fps_pis_hpb_m_extended.stats = {concealment = -2, extra_ammo= 7} 
  self.parts.wpn_fps_pis_hpb_comp.stats = {concealment = -1, recoil = -8} 
  self.parts.wpn_fps_pis_hpb_comp2.stats = {concealment = -1, recoil = -8}
end
--QBZ-97B
 if BeardLib.Utils:ModLoaded("QBZ-97B") then
 self.parts.wpn_fps_ass_qbz97b_95b_body.stats = deep_clone(nostats)
 self.parts.wpn_fps_ass_qbz97b_mag_short.stats = {reload = 30, concealment = 2, extra_ammo= - 10}
...
--AUG 9mm Conversion Kitt / AUG Para kit
if BeardLib.Utils:ModLoaded("AUG 9mm") then
self.parts.wpn_fps_ass_aug_m_9mm.stats = { damage = -5, recoil = 15, spread = -10, concealment = 2, ammo_pickup_min_mul = 1.5,
		ammo_pickup_max_mul = 1.5}
self.parts.wpn_fps_ass_aug_m_9mm.override_weapon = { 
CLIP_AMMO_MAX = 32, AMMO_MAX = 225,} 
end

--TOZ-106
if BeardLib.Utils:ModLoaded("TOZ-106") then
self.parts.wpn_fps_shot_toz106_body_black.stats = deep_clone(nostats)
self.parts.wpn_fps_shot_toz106_grip_ak101.stats = deep_clone(nostats)
self.parts.wpn_fps_shot_toz106_b_long.stats = deep_clone(barrel_m2)
self.parts.wpn_fps_shot_toz106_m_2.stats = { concealment = 2, extra_ammo = -1, reload = 65}
self.parts.wpn_fps_shot_toz106_m_5.stats = { concealment = -2, extra_ammo = 2, reload = -20}
self.parts.wpn_fps_shot_toz106_stock_full.stats = { concealment = -2, recoil = 10}
self.parts.wpn_fps_shot_toz106_stock_mosin.stats = { concealment = -4, recoil = 15}
self.parts.wpn_fps_shot_toz106_stock_folded.stats = { concealment = 2, recoil = -5}
self.parts.wpn_fps_shot_toz106_stock_no.stats = { concealment = 4, recoil = -13}
end
--PN23 Night Vision
if BeardLib.Utils:ModLoaded("PN23 Night Scope") then
self.parts.wpn_fps_upg_o_pn23.stats = {zoom = 8, concealment = -4}
end
--Spectre M4
	if BeardLib.Utils:ModLoaded("Spectre M4") then
		self.parts.wpn_fps_smg_spectre_m4_m_standard.stats = { concealment = -1, extra_ammo = 20}
		self.parts.wpn_fps_smg_x_spectre_m4_m_standard.stats = { concealment = -1, extra_ammo = 40}
		self.parts.wpn_fps_smg_spectre_m4_no_stock.stats = { concealment = 3, recoil= -15}
		self.parts.wpn_fps_smg_spectre_m4_no_vg.stats = { concealment = 2, recoil= -8}
	end
--Vickers
	if BeardLib.Utils:ModLoaded("Vickers MMG") then
		self.parts.wpn_fps_lmg_vick_b_antiair.stats = deep_clone{nostats}
		self.parts.wpn_fps_lmg_vick_m_box.stats = {reload = 65, concealment = - 2, recoil = -5}
		self.parts.wpn_fps_lmg_vick_m_box.custom_stats = { movement_speed = 0.95}
		self.parts.wpn_fps_lmg_vick_b_smooth.stats = deep_clone{nostats}
		self.parts.wpn_fps_lmg_vick_bipod.adds = {"inf_bipod_part"}
		self.parts.wpn_fps_lmg_vick_bipod.type = "bipod"
		self.parts.wpn_fps_lmg_vick_bipod.custom_stats = {recoil_horizontal_mult = 2}
		self.parts.wpn_fps_lmg_vick_bipod.stats = {
		value = 0,
		concealment = -1
	}
...
--SMLE
	if BeardLib.Utils:ModLoaded("SMLE") then
		self.parts.wpn_fps_snp_enfield_no4i_mag_trench.stats = {extra_ammo = 10, concealment = -4}
		self.parts.wpn_fps_snp_enfield_no4i_forend_no5i.stats = {concealment = 6, spread = -10, recoil = -10}
		self.parts.wpn_fps_snp_enfield_no4i_cheek_rest.stats = deep_clone{nostats}
		self.parts.wpn_fps_snp_enfield_no4i_mag_short.stats = {concealment = 2, reload = 65}
		self.parts.wpn_fps_snp_enfield_no4i_md_no5i.stats = deep_clone{nostats}
		self.parts.wpn_fps_snp_enfield_no4i_stock_pad.stats = deep_clone{nostats}
		self.parts.wpn_fps_snp_enfield_no5i_md_none.stats = deep_clone{nostats}
		self.parts.wpn_fps_snp_enfield_no5i_mag_short.stats = {concealment = 2, reload = 65}
		self.parts.wpn_fps_snp_enfield_no5i_forend_no4i.stats = {concealment = -6, spread = 10, recoil = 10}
		self.parts.wpn_fps_snp_enfield_no5i_stock_pad.stats = deep_clone{nostats}
		self.parts.wpn_fps_snp_enfield_no5i_cheek_rest.stats = deep_clone{nostats}
		self.parts.wpn_fps_snp_enfield_no5i_scope.stats = {zoom = 7, concealment = -2}
	end
--KSVK
	if BeardLib.Utils:ModLoaded("KSVK Anti-Materiel Rifle") then
		self.parts.wpn_fps_snp_carpet_fg_wooden.stats = deep_clone(nostats)
		self.parts.wpn_fps_snp_carpet_ns_cans.stats = deep_clone(silstatsconc0)
		self.parts.wpn_fps_snp_carpet_ns_preved.stats = deep_clone(barrel_m2)
		self.parts.wpn_fps_snp_carpet_carry_handle.stats = {reload = 25, concealment = -2}
		self.parts.wpn_fps_snp_carpet_ns_vssk.stats = deep_clone(silstatssnp)
	end
--DSR-1
	if BeardLib.Utils:ModLoaded("DSR-1") then
		self.parts.wpn_fps_snp_bigbust_b_short.stats = {concealment = -2, spread = -5, recoil = 0}
		self.parts.wpn_fps_snp_bigbust_bipod_none.stats = {concealment = 2}
		self.parts.wpn_fps_snp_bigbust_supp.stats = deep_clone(silstatssnp)
		end
--M240 Various Attachment
	if BeardLib.Utils:ModLoaded("M240 Various Attachment") then
		self.parts.wpn_fps_lmg_par_fg_heat.stats = deep_clone(nostats)
		self.parts.wpn_fps_lmg_par_fg_zero.stats = deep_clone(nostats)
		self.parts.wpn_fps_lmg_par_s_early.stats = deep_clone(nostats)
		self.parts.wpn_fps_lmg_par_m_can.stats = {concealment = - 4, extra_ammo = 100, reload = -35}self.parts.wpn_fps_lmg_par_m_can.custom_stats = {}
	end
