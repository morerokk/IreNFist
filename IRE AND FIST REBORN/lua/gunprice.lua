...
if InFmenu.settings.changeitemprices then
	function MoneyManager:get_weapon_price(weapon_id)
		...
		-- fuck you my price now
		if tweak_data.weapon[weapon_id] and tweak_data.weapon[weapon_id].price then
			cost = tweak_data.weapon[weapon_id].price
		end
	...
	function MoneyManager:get_weapon_price_modified(weapon_id)
		...
		local cost = self:get_tweak_value("money_manager", "weapon_cost", pc)
		-- fuck you my price now
		if tweak_data.weapon[weapon_id].price then
			cost = tweak_data.weapon[weapon_id].price
		end
